#!/bin/bash
# ScanNet++ combined SfM + 3DGS per (scene, arm). Task -> (scene, conf) via
# scene = SCENES[id % n], conf = CONFS[id / n] with n = #uncommented SCENES.
# Currently n=21 (small subset) and 5 arms -> submit:
#   sbatch --array=0-104%16 scripts/slurm/scannetpp_array.sh
# Retry one task: sbatch --array=<idx> [overrides] scripts/slurm/scannetpp_array.sh
# (mediums/larges commented out; larges need --time=96:00:00 --mem=128G)
#
...
# Aug 16 arms: A = maxmix + min-over-modes BA gates (new code, fresh name —
# pre-gates base + maxmix results already on disk from the Aug 12 fleet);
# B/C = integrate-off matched pair (unimodal vs maxmix+gates);
# D/E = engagement pair (constant 1% depth uncertainty, integrate off).
CONFS=(repr-sp-lg_m3dv2-maxmix-gates
       repr-sp-lg_m3dv2-noint
       repr-sp-lg_m3dv2-maxmix-gates-noint
       repr-sp-lg_m3dv2-uconst-noint
       repr-sp-lg_m3dv2-maxmix-gates-uconst-noint)
